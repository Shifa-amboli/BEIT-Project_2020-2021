...
mysql_select_db("details"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysql_query("SELECT * FROM `details`.`plogin` WHERE `Username`='$Username'");
			if(mysql_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysql_query("UPDATE `details`.`plogin` SET `Password` = '$Password' WHERE `plogin`.`Username` = '$Username'"))
					{
